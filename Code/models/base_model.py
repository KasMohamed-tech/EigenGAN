...
class BaseModel(ABC):
    ...
    def writer_add_image(self, name, tensor_img, detach=False):
        ...
        img = self.tensor2im(tensor_img[0])
        # print(img.shape)
    ...
    def make_grid(self, feat_img):

        n_rows = feat_img.shape[0]//8 

        for i in range(n_rows): 
            if i == 0 : 
                out_img = self.get_row_image(feat_img[8*i:8*(i+1)-1,:,:])
            else : 
                out_img = out_img + self.get_row_image(feat_img[8*i:8*(i+1)-1,:,:])
        return out_img
    
    def get_row_image(self, feat_img):


        for j in range(feat_img.shape[0]):
            if j == 0 :
                out_row = 255*(feat_img[j,:,:])/np.max(feat_img[j,:,:])
            else : 
                out_row = np.vstack((out_row, feat_img[j,:,:]))
